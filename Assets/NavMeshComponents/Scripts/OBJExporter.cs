...
public class OBJExporter : ScriptableWizard
{
    ...
    void OnWizardCreate()
    {
        if (StaticBatchingEnabled() && Application.isPlaying)
        {
            EditorUtility.DisplayDialog("Error",
                "Static batching is enabled. This will cause the export file to look like a mess, as well as be a large filesize. Disable this option, and restart the player, before continuing.",
                "OK");
            goto end;
        }
        ...
        {
            int yes = EditorUtility.DisplayDialogComplex("Warning",
                "This will convert all textures to Advanced type with the read/write option set. This is not reversible and will permanently affect your project. Continue?",
                "Yes", "No", "Cancel");
            if (yes > 0)
            {
                goto end;
        ...
        var exportName = UnityEngine.SceneManagement.SceneManager.GetActiveScene().name;
        var exportFolder = Application.dataPath;// + "/../../../u3d_config/config_dev/meshes";
        var exportPath = exportFolder + "/" + exportName + ".obj";
        Export(exportPath);
    ...
    void Export(string exportPath)
    {
        ...
        exportFolder = exportFileInfo.Directory.FullName;
            ...
            foreach (MeshFilter mf in sceneMeshes)
            {
                ...
                if (mr != null)
                {
                    if (mr.isPartOfStaticBatch)
                    {
                        ...
                        EditorUtility.DisplayDialog("Error",
                            "Static batched object detected. Static batching is not compatible with this exporter. Please disable it before starting the player.",
                            "OK");
                        return;
                    }
                }
        ...
        for (int i = 0; i < sceneMeshes.Length; i++)
        {
            ...
            EditorUtility.DisplayProgressBar("Exporting objects... (" + Mathf.Round(progress * 100) + "%)",
                "Exporting object " + meshName, progress);

            int objLayer = sceneMeshes[i].gameObject.layer;
            // if (objLayer != LayerMask.NameToLayer("Terrain") && objLayer != LayerMask.NameToLayer("Obstacle"))
            //     continue;
            ...
            int faceOrder =
                (int)Mathf.Clamp((mf.gameObject.transform.lossyScale.x * mf.gameObject.transform.lossyScale.z), -1, 1);
            ...
            for (int j = 0; j < msh.subMeshCount; j++)
            {
                ...
                for (int t = 0; t < tris.Length; t += 3)
                {
                    ...
                    if (faceOrder < 0)
                    {
                        sb.AppendLine("f " + ConstructOBJString(idx2) + " " + ConstructOBJString(idx1) + " " +
                                      ConstructOBJString(idx0));
                    }
                    else
                    {
                        sb.AppendLine("f " + ConstructOBJString(idx0) + " " + ConstructOBJString(idx1) + " " +
        ...
        if (generateMaterials)
        {
            System.IO.File.WriteAllText(exportFileInfo.Directory.FullName + "\\" + baseFileName + ".mtl",
    ...
    [MenuItem("UGameTools/SceneTools/32.Export Wavefront OBJ", false, 32)]
